...
export const calculateAdvancedLeadScore = async (lead: Lead): Promise<LeadScore> => {
  try {
    const breakdown = {
...
const calculateUrgencyScore = (timeline: string): number => {
  switch (timeline) {
    case '1-3 months': return 20; // Immediate need
    case '3-6 months': return 15; // Near-term
    case '6+ months': return 10;  // Future planning
    default: return 5;
  }
};
...
const calculateContactScore = (email: string, phone: string): number => {
  ...
  if (phone.startsWith('+639')) score += 10; // Proper international format
  else if (phone.startsWith('09')) score += 8; // Local format
...
const calculateSourceScore = (source: string): number => {
  switch (source) {
    case 'Referral': return 15; // Highest quality
    case 'Website': return 12;  // Direct interest
    case 'Email Campaign': return 10; // Engaged with marketing
    case 'Social Media': return 8; // Social engagement
    case 'Open House': return 6; // Casual interest
    default: return 3;
  }
};
...
export const assignLeadToAgent = async (lead: Lead, agents: Agent[], leadScore: LeadScore): Promise<AgentAssignment> => {
  try {
...
const findBestAgent = (lead: Lead, agents: Agent[], leadScore: LeadScore): Agent | null => {
...
const calculateAgentMatchScore = (lead: Lead, agent: Agent, leadScore: LeadScore): number => {
  let score = 0;

  // Specialization match
  const leadType = lead.budget > 5000000 ? 'Luxury' : 
                   lead.budget > 3000000 ? 'Commercial' : 'Residential';
  
  if (agent.specialization === leadType) score += 30;
  else if (agent.specialization === 'Luxury' && leadType === 'Commercial') score += 20;
...
const generateMatchReasons = (lead: Lead, agent: Agent, leadScore: LeadScore): string[] => {
  const reasons = [];
...
const notifyAgentOfAssignment = async (agent: Agent, lead: Lead, leadScore: LeadScore): Promise<void> => {
  try {
...
export const batchProcessLeadScoring = async (leads: Lead[]): Promise<LeadScore[]> => {
  try {
    const scoringPromises = leads.map(lead => calculateAdvancedLeadScore(lead));
...
export const updateLeadScore = async (leadId: string, newData: Partial<Lead>): Promise<LeadScore> => {
  try {
    ...
    return {
      ...
      priority: 'High',
      estimatedCloseTime: new Date(Date.now() + 60 * 24 * 60 * 60 * 1000).toISOString().split('T')[0], // 60 days
