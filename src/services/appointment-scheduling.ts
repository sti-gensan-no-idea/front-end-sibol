...
export const scheduleAppointment = async (
  ...
  try {
    ...
    const eventData: EventScheduleData = {
      ...
      location: request.propertyId
        ? `Property ${request.propertyId}`
        : "aTuna Office",
      type: request.type,
    };

    const openMaintResponse = await apiClients.openMaint.post(
      "/events",
      eventData
    );
    ...
    return {
      success: false,
      message:
        "Failed to schedule appointment. Please try again or contact us directly.",
    };
  }
};
...
const generateConfirmationMessage = (
  ...
  const location = request.propertyId
    ? "the property location"
    : "our office at Pioneer Avenue, General Santos City";
  ...
  try {
    const reminders = [
      ...
      },
      {
        ...
        type: "sms",
        message:
          "Tomorrow: Property viewing at ${time}. Reply CONFIRM or RESCHEDULE",
      },
      {
        hours: 2,
        type: "sms",
        message:
          "In 2 hours: Property viewing. Address: ${location}. Agent: ${agent}",
      },
    ];
    ...
    console.log(
      "Automated reminders scheduled for appointment:",
...
export const rescheduleAppointment = async (
  appointmentId: string,
  newRequest: Partial<AppointmentRequest>
): Promise<SchedulingResponse> => {
  try {
...
export const createOpenHouseEvent = async (
  propertyId: string,
  date: string,
  time: string
): Promise<ApiResponse> => {
  try {
    ...
    const response = await apiClients.openMaint.post(
      "/events/open-house",
      eventData
    );
...
const createOpenHousePromotion = async (
  propertyId: string,
  date: string,
  time: string
): Promise<void> => {
...
export const bulkScheduleViewings = async (
  propertyId: string,
  leadIds: string[]
): Promise<ApiResponse[]> => {
  try {
    ...
    console.log(
      `Bulk scheduled ${results.length} viewings for property ${propertyId}`
    );
...
export const getAgentAvailability = async (
  ...
  try {
    const response = await apiClients.openMaint.get(
...
const findBestAgentForAppointment = (
