import { apiClients, ApiResponse, PropertyListing, LeadCaptureData } from './api-config';
...
export const captureLeadFromProperty = async (leadData: LeadCaptureData): Promise<ApiResponse> => {
  try {
...
export const sendWelcomeMessage = async (leadData: LeadCaptureData): Promise<ApiResponse> => {
  try {
...
export const sendWelcomeEmail = async (leadData: LeadCaptureData): Promise<ApiResponse> => {
  try {
...
export const scheduleFollowUpSequence = async (leadId: string, leadData: LeadCaptureData): Promise<ApiResponse> => {
  try {
    ...
    console.log('Follow-up sequence scheduled for lead:', leadId, followUpSchedule);
...
const calculateLeadScore = (leadData: LeadCaptureData): number => {
  ...
  if (leadData.preferences?.budget) {
    if (leadData.preferences.budget > 5000000) score += 30; // High-end PHP 5M+
    else if (leadData.preferences.budget > 3000000) score += 20; // Mid-range PHP 3-5M
    else if (leadData.preferences.budget > 1500000) score += 15; // Entry PHP 1.5-3M
    else score += 5; // Budget-conscious
  }

  // Contact quality scoring
  if (leadData.phone.startsWith('+639')) score += 10; // Valid PH mobile
  if (leadData.email.includes('.com') || leadData.email.includes('.ph')) score += 5;
...
const generateWelcomeEmailContent = (leadData: LeadCaptureData): string => {
  const budget = leadData.preferences?.budget ? `₱${(leadData.preferences.budget / 1000000).toFixed(1)}M` : 'your budget';
...
export const triggerBehavioralFollowUp = async (leadId: string, behavior: string, data: any): Promise<ApiResponse> => {
  try {
    let message = '';
    let channel = 'email';
    ...
    console.log(`Behavioral follow-up triggered: ${behavior}`, { leadId, channel, message });
