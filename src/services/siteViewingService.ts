...
export class SiteViewingService {
  ...
  async createSiteViewing(viewing: SiteViewingCreate): Promise<SiteViewingResponse> {
  ...
  // Update site viewing
  async updateSiteViewing(id: string, updates: SiteViewingUpdate): Promise<SiteViewingResponse> {
    return apiService.put(`/site-viewings/${id}`, updates);
  }

  // Cancel site viewing
  async cancelSiteViewing(id: string, reason?: string): Promise<SiteViewingResponse> {
  ...
  async completeSiteViewing(id: string, notes?: string): Promise<SiteViewingResponse> {
  ...
  async assignAgentToViewing(viewingId: string, agentId: string): Promise<SiteViewingResponse> {
    return apiService.put(`/site-viewings/${viewingId}/assign-agent`, { agent_id: agentId });
  }

  // Get available time slots for property
  async getAvailableTimeSlots(propertyId: string, date: string): Promise<string[]> {
    return apiService.get(`/site-viewings/available-slots?property_id=${propertyId}&date=${date}`);
  }

  // Reschedule site viewing
  async rescheduleSiteViewing(id: string, newDate: string): Promise<SiteViewingResponse> {
    return apiService.put(`/site-viewings/${id}/reschedule`, { scheduled_date: newDate });
  }
}
