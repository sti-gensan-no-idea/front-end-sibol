...
const IntegratedDashboard: React.FC = () => {
  ...
  } = useAnalytics({ autoFetch: true, refreshInterval: 5 * 60 * 1000 });
  ...
  const handleCSVUpload = async () => {
    if (csvFile) {
      ...
      if (result) {
        console.log(`Upload completed: ${result.success_count} properties created`);
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      <h1 className="text-3xl font-bold mb-8">Atuna Backend Integration Dashboard</h1>
        ...
        {analyticsLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="bg-white p-4 rounded-lg shadow">
              ...
              <p className="text-3xl font-bold text-green-600">{dashboardData.active_users}</p>
            </div>
            <div className="bg-white p-4 rounded-lg shadow">
              <h3 className="text-lg font-medium">Total Revenue</h3>
              <p className="text-3xl font-bold text-purple-600">₱{dashboardData.total_revenue.toLocaleString()}</p>
            </div>
            <div className="bg-white p-4 rounded-lg shadow">
              <h3 className="text-lg font-medium">Monthly Revenue</h3>
              <p className="text-3xl font-bold text-orange-600">₱{dashboardData.monthly_revenue.toLocaleString()}</p>
            </div>
          </div>
        ) : (
      ...
      {/* Properties Section */}
      <section className="mb-8">
        <h2 className="text-2xl font-semibold mb-4">Properties ({properties.length})</h2>
        <div className="flex gap-4 mb-4">
          ...
          <div className="flex gap-2">
            ...
            />
            <Button onClick={handleCSVUpload} disabled={!csvFile} color="secondary">
              Upload CSV
            </Button>
          </div>
        </div>
      ...
      <section className="mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <Button onClick={handleCreateEvent} color="primary" className="w-full">
            Create Sample Event
          </Button>
          <Button onClick={handleCreateLead} color="secondary" className="w-full">
            Create Sample Lead
          </Button>
          <Button onClick={handleScheduleSiteViewing} color="success" className="w-full">
            Schedule Site Viewing
          </Button>
          <Button onClick={handleMoveLead} disabled={leads.length === 0} color="warning" className="w-full">
            Move Lead Stage
          </Button>
        </div>
      </section>
      ...
      <section className="mb-8">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div className="bg-white p-4 rounded-lg shadow">
            ...
            <p className="text-2xl font-bold text-purple-600">{siteViewings.length}</p>
            <p className="text-sm text-gray-500">Scheduled viewings</p>
          </div>
          <div className="bg-white p-4 rounded-lg shadow">
            <h3 className="text-lg font-medium">Properties</h3>
            <p className="text-2xl font-bold text-orange-600">{properties.length}</p>
      ...
      <div className="text-center text-gray-600 mt-8 p-6 bg-green-50 rounded-lg">
        <h3 className="text-xl font-semibold text-green-800 mb-2">✅ Integration Complete!</h3>
        <p className="mb-2">All Atuna backend API endpoints have been successfully integrated with the React frontend.</p>
        <p className="text-sm">
          <strong>Backend Services:</strong> Properties, Teams, Events, Site Viewings, CRM, Analytics, Maintenance, Payments
        </p>
        <p className="text-sm">
          <strong>React Hooks:</strong> Type-safe hooks for all API operations with error handling and loading states
        </p>
        <p className="mt-4 text-green-700">
          Check the browser console for API responses when you use the action buttons above.
        </p>
      </div>
    </div>
  );
