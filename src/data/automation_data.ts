...
export interface Lead {
  ...
  source: 'Website' | 'Social Media' | 'Referral' | 'Email Campaign' | 'Open House';
  seriousness: number;
  preferences: {
    petFriendly: boolean;
    nonHaunted: boolean;
...
export interface Event {
  ...
  date: string;
  time: string;
  type: 'Viewing' | 'Inspection' | 'Lease Review' | 'Document Signing' | 'Open House';
  client: string;
  agent: string;
  propertyId: string;
...
const neighborhoods = ['Rizal St', 'Quezon Ave', 'Magsaysay Blvd', 'Pioneer Ave', 'Dacera St'];
const streetNumbers = Array.from({ length: 50 }, (_, i) => 100 + i);

export const sampleProperties: Property[] = Array.from({ length: 50 }, (_, i) => ({
  id: `prop-${i + 1}`,
  ...
  status: ['Available', 'Pending', 'Sold'][Math.floor(Math.random() * 3)] as 'Available' | 'Pending' | 'Sold',
...
export const sampleLeads: Lead[] = Array.from({ length: 50 }, (_, i) => ({
  ...
  timeline: ['1-3 months', '3-6 months', '6+ months'][Math.floor(Math.random() * 3)] as '1-3 months' | '3-6 months' | '6+ months',
  source: ['Website', 'Social Media', 'Referral', 'Email Campaign', 'Open House'][i % 5] as 'Website' | 'Social Media' | 'Referral' | 'Email Campaign' | 'Open House',
  ...
  status: ['New', 'Contacted', 'Qualified', 'Converted'][Math.floor(Math.random() * 4)] as 'New' | 'Contacted' | 'Qualified' | 'Converted',
  lastContact: new Date(2025, 6, Math.floor(Math.random() * 30) + 1).toISOString().split('T')[0],
...
export const sampleAgents: Agent[] = Array.from({ length: 50 }, (_, i) => ({
  ...
  specialization: ['Residential', 'Commercial', 'Luxury'][Math.floor(Math.random() * 3)] as 'Residential' | 'Commercial' | 'Luxury',
...
export const sampleMessages: Message[] = Array.from({ length: 50 }, (_, i) => ({
  id: `msg-${i + 1}`,
  content: i % 4 === 0 ? `Open house at ${streetNumbers[i]} ${neighborhoods[i % 5]}` :
           i % 4 === 1 ? `Thank you for inquiring about ${streetNumbers[i]} ${neighborhoods[i % 5]}!` :
           i % 4 === 2 ? `Your lease review is scheduled for ${new Date(2025, 7, i % 28 + 1).toLocaleDateString()}` :
           `Property viewing available for ${streetNumbers[i]} ${neighborhoods[i % 5]}`,
  channel: ['SMS', 'Email', 'Social Media', 'Chat'][i % 4] as 'SMS' | 'Email' | 'Social Media' | 'Chat',
  recipient: `client${(i % 50) + 1}@atuna.com`,
  sender: `agent${(i % 50) + 1}@atuna.com`,
  sentiment: ['Positive', 'Neutral', 'Negative'][Math.floor(Math.random() * 3)] as 'Positive' | 'Neutral' | 'Negative',
  timestamp: new Date(2025, 6, Math.floor(Math.random() * 30) + 1).toISOString(),
...
export const sampleEvents: Event[] = Array.from({ length: 50 }, (_, i) => ({
  ...
  time: `${9 + (i % 8)}:00`,
  type: ['Viewing', 'Inspection', 'Lease Review', 'Document Signing', 'Open House'][i % 5] as 'Viewing' | 'Inspection' | 'Lease Review' | 'Document Signing' | 'Open House',
...
export const sampleLeases: Lease[] = Array.from({ length: 50 }, (_, i) => ({
  ...
  status: ['Active', 'Pending', 'Expired'][Math.floor(Math.random() * 3)] as 'Active' | 'Pending' | 'Expired',
  riskLevel: ['Low', 'Medium', 'High'][Math.floor(Math.random() * 3)] as 'Low' | 'Medium' | 'High',
  paymentHistory: ['Good', 'Late', 'Default'][Math.floor(Math.random() * 3)] as 'Good' | 'Late' | 'Default'
}));

export const sampleDocuments: Document[] = Array.from({ length: 50 }, (_, i) => ({
  id: `doc-${i + 1}`,
  type: ['Lease Agreement', 'Contract', 'Disclosure', 'Inspection Report'][Math.floor(Math.random() * 4)] as 'Lease Agreement' | 'Contract' | 'Disclosure' | 'Inspection Report',
  clientEmail: `client${(i % 50) + 1}@atuna.com`,
  propertyId: `prop-${(i % 50) + 1}`,
  status: ['Pending', 'Signed', 'Rejected'][Math.floor(Math.random() * 3)] as 'Pending' | 'Signed' | 'Rejected',
  createdDate: new Date(2025, 6, Math.floor(Math.random() * 30) + 1).toISOString().split('T')[0],
  signedDate: Math.random() > 0.5 ? new Date(2025, 7, Math.floor(Math.random() * 30) + 1).toISOString().split('T')[0] : undefined
}));
...
export const calculateAnalytics = () => {
  ...
  const conversionRate = (highSeriousnessLeads.length / sampleLeads.length * 100);

  const leadSourceCounts = sampleLeads.reduce((acc, lead) => {
    acc[lead.source] = (acc[lead.source] || 0) + 1;
    return acc;
  }, {} as Record<string, number>);

  const neighborhoodData = sampleProperties.reduce((acc, property) => {
    const neighborhood = property.address.split(' ')[1] + ' ' + property.address.split(' ')[2];
    if (!acc[neighborhood]) {
      acc[neighborhood] = { 
        total: 0, 
        converted: 0, 
  ...
  return {
    ...
    scheduledEvents: sampleEvents.filter(e => e.status === 'Scheduled').length,
    activeLeases: sampleLeases.filter(l => l.status === 'Active').length,
    averageAgentRating: sampleAgents.reduce((sum, a) => sum + a.rating, 0) / sampleAgents.length,
    automatedMessages: sampleMessages.filter(m => m.automated).length,
    positiveMessages: sampleMessages.filter(m => m.sentiment === 'Positive').length
  };
};
